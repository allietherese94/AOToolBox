...
To run all the behaviors you wish to record during focal observations, upload the necessary ".csv" files using the panel on the left. This allows you to generate the **"behaviors.json"** file that is used by the app. This information will be displayed as pop-windows during a focal. A minimum of 3 files is needed to generate **"behaviors.json"**. 

1. Dyadic interactions, recorded between scans.  
2. Animal activites, recorded during scans.  
...
* If the identity of another individual is to be recorded for a certain activity, this *must* be specified in one of the columns with the value _ID. This refers to the group composition ".csv" file (See <a id="link_to_animals" href="#" class="action-button">Study animals</a> for specifications) which specifies all individuals being studied. 
...
To keep track of which version of your protocol files were used for collected data, specify the version name or number as required. This can be any string of characters. When the protocol files are generated, the version will be saved along with the focal data. 

(Note that this is not the same as the version of the application!)

<hr size="5">

#### Add behaviors.json file to iPad

Once all these ".csv" files have been uploaded in the left panel, click on "Run" and the **"behaviors.json"** file will be generated. Click on "Download behaviors.json" to save the file and upload it to the iPad following <a id="link_to_welcome" href="#" class="action-button">these instructions</a>.
